...
def bootstrap(host, port, apps, log_conf={}):
    httpd = SEngineEpollHttpServer(host, port, log_conf, apps)
    httpd.serve_forever()


def bootstrap_with_driven(host, port, driven_process, log_conf={}):
    if driven_process is None:
        print('none driven_process')
        raise Exception('none driven_process')
    httpd = SEngineEpollHttpServer(host, port, log_conf, None, driven_process)
    httpd.serve_forever()


class SEngineEpollHttpServer:
